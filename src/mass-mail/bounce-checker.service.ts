...
export class BounceCheckerService implements OnModuleDestroy {
    ...
    async checkForBounces() {
        ...
        try {
            ...
            try {
                ...
                this.logger.log(`Found ${messages.length} potential bounce message(s).`);
                ...
                for (const uid of messages) {
                    try {
                        ...
                        this.logger.error(`Error processing bounce message UID ${uid}:`, msgError.message);
                    }
                }

                this.logger.log(`Bounce check complete: ${processed} of ${messages.length} messages resulted in status updates.`);
     ...
     */
    private parseBounceEmail(parsed: any): { failedRecipient: string; errorMessage: string } | null {
        ...
        const finalRecipientMatch = fullText.match(/Final-Recipient:\s*rfc822;\s*([^\s\r\n]+)/i);
        ...
        if (!failedRecipient) {
            const origRecipientMatch = fullText.match(/Original-Recipient:\s*rfc822;\s*([^\s\r\n]+)/i);
        ...
        if (!failedRecipient) {
            this.logger.debug(`Could not extract recipient from bounce. Subject: "${subject}"`);
            return null;
        }

        // --- Step 4: Extract the error/diagnostic message ---
        let errorMessage = 'Correo rebotado — dirección no válida o buzón inexistente';
        ...
        const diagMatch = fullText.match(/Diagnostic-Code:\s*smtp;\s*([\s\S]*?)(?:\n[A-Z]|\n\n|$)/i);
        if (diagMatch) {
        ...
        // Priority 2: "said: 550..." from text/plain body
            ...
            const saidMatch = textBody.match(/said:\s*([\s\S]*?)(?:\(in reply to|\n\n)/i);
            if (saidMatch) {
     ...
     */
    private async updateEmailLog(recipient: string, errorMessage: string): Promise<boolean> {
        ...
        if (findError) {
            this.logger.error(`Error finding email log for ${recipient}:`, findError.message);
            return false;
        }

        if (!logs || logs.length === 0) {
            this.logger.debug(`No matching 'sent' email log found for bounced recipient: ${recipient}`);
            return false;
        }
        ...
        if (updateError) {
            this.logger.error(`Error updating email log ${logId} to bounced:`, updateError.message);
            return false;
        }

...
let UsersController = class UsersController {
    ...
    async createAuthUser(body) {
        if (!body.email || !body.password) {
            throw new common_1.HttpException('Email y Password son requeridos', common_1.HttpStatus.BAD_REQUEST);
        }
        return await this.usersService.createAuthUser(body.email, body.password);
    }
    async changeUserPassword(body) {
        if (!body.userId || !body.newPassword) {
            throw new common_1.HttpException('userId y newPassword son requeridos', common_1.HttpStatus.BAD_REQUEST);
        }
...
], UsersController.prototype, "createAuthUser", null);
__decorate([
